 ...
 * @version: 1.0
 * @Author: weipeng
 * @Date: 2022-06-03 09:07:48
 * @LastEditors: weipeng
 * @LastEditTime: 2022-06-03 09:43:37
 */
/*
力扣649. Dota2 参议院
...
// 贪心策略：每一轮中，每一个字母消灭其后面的不同字母
class Solution {
public:
    string predictPartyVictory(string senate) {
        // ture表示当前轮次结束后字符串中仍有该字母
        bool R = true, D = true;
        // flag > 0表示R在D前面，可以消除D；反之表示D在R前面，应该消除R
        int flag = 0;
        // 任意一个为false表示字符串中没有该字母了，结束循环
        while (R && D) {
            R = false;
            D = false;
